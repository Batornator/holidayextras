...
class User {
    ...
    return new Promise( ( resolve, reject ) => {
      ...
      db.withinTransaction( ( transaction ) => {
        return new Promise( ( transactionResolve, transactionReject ) => {
          ...
          transaction.query( createSql, createValues, ( err, result ) => {
            if ( err ) {
              return transactionReject( errorUtils.modelSaveError( "Failed to save User", err ) );
            }

            return User.findById( result.insertId, transaction ).then( transactionResolve ).catch( transactionReject );
          } );
        } );
      } )
        .then( ( foundUser ) => {
        ...
        .catch( ( err ) => {
          if ( err ) {
            return reject( errorUtils.genericSqlError( "Failed to save User", err ) );
          }
        } );
    } );
  }
  ...
  static async findById( id, transaction ) {
    return new Promise( ( resolve, reject ) => {
      ...
      ( transaction || db ).query( findSql, [ id ], ( err, result ) => {
        if ( err ) {
  ...
  static async updateById( id, updates ) {
    return new Promise( ( resolve, reject ) => {
      ...
      db.withinTransaction( ( transaction ) => {
        return new Promise( ( transactionResolve, transactionReject ) => {
          transaction.query( updateSql, [ mappedUpdates, id ], ( err, result ) => {
            if ( err ) {
            ...
            User.findById( id, transaction ).then( transactionResolve ).catch( transactionReject );
          } );
        } );
      } )
        .then( ( foundUser ) => {
